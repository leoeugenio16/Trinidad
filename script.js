...
function updateCountdown(){
  ...
  if(diff <= 0){
    document.getElementById('countdown').innerHTML = '<p class="hero-text">¡Llegó el gran día!</p>';
    return;
  }
...
musicBtn.addEventListener('click', async()=>{
  try{
    if(!playing){ await music.play(); musicBtn.textContent = '❚❚ Pausar'; }
    else{ music.pause(); musicBtn.textContent = '♫ Música'; }
    playing = !playing;
  }catch(e){ alert('Agregá un archivo musica.mp3 en la carpeta assets para activar la música.'); }
});
...
if (form) {
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      form.reset();
      formMessage.textContent = "¡Confirmación enviada con éxito!";
      formMessage.className = "form-message ok";
    } catch (error) {
      formMessage.textContent = "No se pudo enviar. Intentá nuevamente.";
      formMessage.className = "form-message error";
    }
  });
}